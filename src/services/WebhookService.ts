...
class WebhookApiService implements IwebhookService {
  ...
  async getAll(): Promise<
    ApiResponse<
      Omit<
        Webhook[],
        "event_type" | "subscription_url" | "object_type" | "object_ids"
      >
    >
  > {
  ...
  async create(
    ...
    return await this.httpService.post("/webhooks", body, {
      Authorization: `Bearer ${token?.value}`,
    });
  }
  ...
  async delete(id: string): Promise<void> {
    const token = cookies().get("smwm_token");
    return await this.httpService.delete(`/webhooks/${id}`, {
      Authorization: `Bearer ${token?.value}`,
    });
  }
...
export const webhookService: IwebhookService = new WebhookApiService(
  fetchService,
);
